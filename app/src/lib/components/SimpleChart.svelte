<!-- Chart, der die Entwicklung einzelner Coins darstellt, ziemlich ähnlich zu PortfolioChart-->
<script lang="ts">
	...
	function handleMouseMove(event) {
		...
		if (dataIndex >= 0 && dataIndex < chartData.length) {
			...
			const pointY =
				padding.top +
				chartHeight -
				((dataPoint.price - Math.min(...chartData.map((d) => d.price))) /
	...
	async function fetchChartData() {
		...
		try {
			...
			const url = `/api/simple-chart?symbol=${symbol.toLowerCase()}&days=${
			...
			if (responseData.isMock) {
				useMockData = true;
				errorMessage =
					responseData.error || 'API-Limit erreicht. Es werden Beispieldaten angezeigt.';
			} else {
				useMockData = false;
			}
