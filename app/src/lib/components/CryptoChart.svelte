...
<script lang="ts">
  ...
  async function fetchChartData() {
    ...
    try {
      // Statt direkter Anfrage an CoinGecko, nutzen wir unseren Server-Proxy
      const url = `/api/crypto-chart?symbol=${symbol.toLowerCase()}&days=${timeframe === '1d' ? '1' : 
                                                                         timeframe === '7d' ? '7' : 
                                                                         timeframe === '30d' ? '30' : 
                                                                         timeframe === '90d' ? '90' : 'max'}`;
      
      const response = await fetch(url);
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || `Fehler beim Abrufen der Kursdaten: ${response.status}`);
      }
      
      const responseData = await response.json();
      
      if (!responseData.success) {
        throw new Error(responseData.message || 'Fehler beim Abrufen der Kursdaten');
      }
      ...
      // Kennzeichnen, wenn Daten simuliert sind
      if (responseData.isMock) {
        error = "API-Limit erreicht. Es werden Beispieldaten angezeigt.";
      }
      ...
      // Fallback: Dummy-Daten generieren, wenn API-Anfragen fehlschlagen
      generateDummyData();
    } finally {
      isLoading = false;
    }
...
<div class="chart-container">
  ...
  <div class="chart-content">
    ...
    {:else if error}
      <div class="error">
        <p>{error}</p>
        <p class="text-sm">Wir zeigen simulierte Daten an, um die Funktionalität zu demonstrieren.</p>
      </div>
    {/if}
